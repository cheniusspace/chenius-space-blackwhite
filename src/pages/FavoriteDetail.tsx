import { useState, useEffect } from "react";
import { useParams, Link } from "react-router-dom";
import { SectionHeading } from "@/components/ui/section-heading";
import { supabase } from "@/integrations/supabase/client";
import { useToast } from "@/hooks/use-toast";
import { ArrowLeft, ExternalLink, Tag as TagIcon, User } from "lucide-react";
import { Button } from "@/components/ui/button";

type Favorite = {
  id: string;
  title: string;
  author: string;
  category: string;
  description: string | null;
  external_link: string | null;
  image_url: string | null;
  tags?: { id: string; name: string }[];
};

const FavoriteDetail = () => {
  const { id } = useParams<{ id: string }>();
  const [favorite, setFavorite] = useState<Favorite | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const { toast } = useToast();
  
  useEffect(() => {
    const fetchFavorite = async () => {
      if (!id) return;
      
      try {
        const { data, error } = await supabase
          .from("favorites")
          .select(`
            *,
            tags:favorites_tags(
              tags(id, name)
            )
          `)
          .eq("id", id)
          .single();
          
        if (error) {
          throw error;
        }
        
        if (data) {
          const normalizedData = {
            ...data,
            tags: data.tags ? data.tags.map((tagItem: any) => tagItem.tags) : []
          };
          
          setFavorite(normalizedData);
        }
      } catch (error) {
        console.error("Error fetching favorite:", error);
        toast({
          title: "Error",
          description: "Failed to load favorite details",
          variant: "destructive",
        });
      } finally {
        setIsLoading(false);
      }
    };
    
    fetchFavorite();
  }, [id, toast]);

  if (isLoading) {
    return (
      <div className="container px-4 md:px-6 max-w-5xl mx-auto py-12 min-h-[60vh] flex items-center justify-center">
        <div className="animate-pulse text-chenius-gray-500">Loading...</div>
      </div>
    );
  }

  if (!favorite) {
    return (
      <div className="container px-4 md:px-6 max-w-5xl mx-auto py-12 min-h-[60vh] flex flex-col items-center justify-center">
        <h1 className="text-2xl font-medium mb-4">Favorite not found</h1>
        <p className="text-chenius-gray-500 mb-6">
          The favorite you are looking for does not exist or has been removed.
        </p>
        <Button asChild>
          <Link to="/favorites" className="flex items-center gap-2">
            <ArrowLeft size={16} />
            Back to Favorites
          </Link>
        </Button>
      </div>
    );
  }

  return (
    <div className="container px-4 md:px-6 max-w-5xl mx-auto py-12">
      <div className="mb-8">
        <Button variant="ghost" asChild className="mb-4 p-0 hover:bg-transparent">
          <Link to="/favorites" className="flex items-center gap-2 text-chenius-gray-500">
            <ArrowLeft size={16} />
            Back to Favorites
          </Link>
        </Button>
        
        <SectionHeading
          title={favorite.title}
          description={favorite.category}
          className="mt-4"
        />
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2">
          {favorite.image_url ? (
            <img 
              src={favorite.image_url} 
              alt={favorite.title} 
              className="w-full h-auto rounded-md mb-6"
            />
          ) : (
            <div className="w-full aspect-video bg-chenius-gray-200 rounded-md mb-6 flex items-center justify-center">
              <p className="text-chenius-gray-500">No image available</p>
            </div>
          )}
          
          <div className="prose prose-chenius max-w-none mt-6">
            {favorite.description ? (
              <p>{favorite.description}</p>
            ) : (
              <p className="text-chenius-gray-500">No description available.</p>
            )}
          </div>
        </div>
        
        <div className="space-y-6">
          <div className="bg-chenius-gray-50 p-6 rounded-md">
            <h3 className="text-lg font-medium mb-4">Details</h3>
            
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                <User className="h-5 w-5 text-chenius-gray-500 mt-0.5" />
                <div>
                  <p className="text-sm text-chenius-gray-500">Author</p>
                  <p className="font-medium">{favorite.author}</p>
                </div>
              </div>
              
              {favorite.external_link && (
                <div>
                  <a 
                    href={favorite.external_link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-800 transition-colors"
                  >
                    Visit External Link
                    <ExternalLink className="h-4 w-4" />
                  </a>
                </div>
              )}
              
              {favorite.tags && favorite.tags.length > 0 && (
                <div className="flex items-start gap-3">
                  <TagIcon className="h-5 w-5 text-chenius-gray-500 mt-0.5" />
                  <div>
                    <p className="text-sm text-chenius-gray-500">Tags</p>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {favorite.tags.map((tag, index) => (
                        <span 
                          key={index} 
                          className="text-xs bg-chenius-gray-100 px-2 py-1 rounded"
                        >
                          {tag.name}
                        </span>
                      ))}
                    </div>
                  </div>
                </div>
              )}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default FavoriteDetail;
