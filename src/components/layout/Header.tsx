  ...
  return (
    <nav className="refined-nav w-full border-b border-white/10 bg-black/90 backdrop-blur-sm">
      <div className="container mx-auto max-w-screen-xl flex justify-between items-center py-4 md:py-8">
        ...
        {/* Navigation Links */}
        <div 
          className={`${isMenuOpen || isAnimating ? 'flex' : 'hidden'} ${isMenuOpen ? 'mobile-menu-enter' : isAnimating ? 'mobile-menu-exit' : ''} md:flex flex-col md:flex-row absolute md:relative top-16 md:top-0 left-0 right-0 bg-black/90 md:bg-transparent p-4 md:p-0 gap-2 md:gap-4 z-50`}
        >
          ...
          <a href="mailto:your-email@example.com" className="refined-nav-link text-white hover:text-white/90 text-sm flex items-center justify-center" aria-label="Contact">
